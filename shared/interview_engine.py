...
- テーマ（topic）の「順番」と「目的」は固定するが、「質問の言い回し・粒度・回数」はAIが
  会話の流れを見て都度決める。
- 暴走防止のため、1テーマあたりの質問数には上限を設ける（各テーマの "max_questions" キーで個別設定）。
...
# DEFAULT_MAX_QUESTIONS はフォールバック用。
# 各テーマは THEMES の "max_questions" キーで個別設定する。
DEFAULT_MAX_QUESTIONS_PER_THEME = 2
...
def judge_and_generate_next_question(
    ...
    """
    max_questions = theme.get("max_questions", DEFAULT_MAX_QUESTIONS_PER_THEME)
    logger.debug(
        "judge_and_generate_next_question: theme=%s asked=%d max=%d",
        theme["key"], questions_asked_in_theme, max_questions,
    )
    if questions_asked_in_theme >= max_questions:
