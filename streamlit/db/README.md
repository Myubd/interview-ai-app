...
- **sessions** — 会社・種別ごとのセッション。進行状態（`progress_state`）・評価結果（`mock_interview_evaluation`）はJSON列に保持
...
### `favorites`
自己PR・模擬面接結果・質問セットなどのお気に入り保存。`item_type` で種別を区別する。
