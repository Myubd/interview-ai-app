...
----------------------------
「お気に入り」機能のCRUD処理を担うリポジトリ。

[Streamlit版 favorites.py からの移植について]
Streamlit版の favorites.py が持つ関数のうち、DBに対するCRUD部分のみを
ここに移植した。build_auto_save_snapshot() は st.session_state に
直接依存する Streamlit 固有のヘルパーのため移植対象外とし、
React版では各サービス層（interview_service 等）が呼び出し元で
snapshot dict を組み立てて add_favorite() に渡す設計とする。
...
テーブル定義は db/database.py の SCHEMA_SQL（favorites テーブル）を参照。
"""
