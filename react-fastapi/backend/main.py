...
| `error` | `{message}` | エラー |
    ...
    },
    {
        "name": "permissions",
        "description": (
            "**データ共有の許可管理**。ライフサポートOS等、他のローカルAIエコシステム\n"
            "アプリとこのアプリの間でメモリー/資料/予定を共有してよいかを、ユーザーが\n"
            "スコープ単位で許可・失効できる。許可しない限り何も共有されない。"
        ),
    },
]
...
async def lifespan(app: FastAPI):
    ...
    except Exception:  # pragma: no cover - 共通基盤側の初期化失敗で本体起動を止めない
